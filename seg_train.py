    ...
    def __init__(self, is_train,root):  # 初始化一些需要传入的参数
        ...
        if self.is_train:
            self.train_tsf = torchvision.transforms.Compose([
                ...
                torchvision.transforms.Normalize(mean=[0.447, 0.359, 0.331], std=[0.180, 0.177, 0.164]),
...
def val( val_loader, model, criterion, best_iou,a):
    ...
    with torch.no_grad():
        ...
        if aver_iou > best_iou:
            ...
            torch.save(model.state_dict(), r'./best_model/seg/RCUNet/flod_'+str(a+1)+'.pth')
...
def train(i,train_iter, test_iter, net, loss, optimizer, device, num_epochs):
    ...
    y5 = train_l_min_l
    plt.plot(x1, y5, 'o-')
    plt.title('Seg Train accuracy vs. epoches')
