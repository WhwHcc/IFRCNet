...
import time
import  csv
from cla import alexnet, mobilenet, resnet, vgg, vgg16, inception,ViT,Mobilenetv4
import numpy as np
import random
from metric import classificationM
import cv2
from seg import IFRCNet,UNet
    ...
    def __init__(self, is_train,root):  # 初始化一些需要传入的参数
        ...
        if self.is_train:
            self.train_tsf = torchvision.transforms.Compose([
                ...
                torchvision.transforms.Normalize(mean=[0.438, 0.438, 0.438], std=[0.287, 0.287, 0.287]),
            ])
            ...
            self.test_tsf = torchvision.transforms.Compose([
                ...
                torchvision.transforms.Normalize(mean=[0.483, 0.397, 0.368], std=[0.287, 0.287, 0.287]),
            ])
...
def k_fold(k,num_epochs,device,batch_size): #,optimizer,loss,net
    ...
    for i in range(k):
        ...
        net = resnet.resnet18() #alexnet.AlexNet()/vgg.vgg16()/resnet.resnet18()/TSCNet.TSCWNet(3,1)
        n_param = sum([np.prod(param.size()) for param in net.parameters()])
        print('Network parameters: ' + str(n_param))
        net = net.cuda()
        net = torch.nn.DataParallel(net)
        net.load_state_dict(torch.load('./best_model/resnet/fold_'+str(i+1)+'.pth'))
