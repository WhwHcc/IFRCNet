...
def evaluate_accuracy(data_iter, net, best_acc, i, loss,device=None):
    ...
    if micro_accuracy > best_acc:
        ...
        torch.save(net.state_dict(), r'./best_model/RCNet/fold_'+str(i+1)+'.pth')
...
def train(i,train_iter, test_iter, net, loss, optimizer, device, num_epochs):
    ...
    plt.savefig('result/losspic/RGE-Net/train_'+str(i+1)+'.png')
    # plt.show()
    ...
    plt.ylabel('Val loss')
    plt.savefig('result/losspic/RCNet/val_' + str(i + 1) + '.png')
    # plt.show()
