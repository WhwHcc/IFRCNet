    ...
    def __init__(self, is_train,root):  # 初始化一些需要传入的参数
        ...
        if self.is_train:
            self.train_tsf = torchvision.transforms.Compose([
                ...
                torchvision.transforms.Normalize(mean=[0.483, 0.397, 0.368], std=[0.173, 0.165, 0.170]),
...
def k_fold(k,image_dir,num_epochs,device,batch_size): #,optimizer,loss,net
    ...
    for i in range(k):
        ...
        test_loader = torch.utils.data.DataLoader(dataset=test_data, batch_size=1, shuffle=True, num_workers=0)

        net = IFRCNet.RCUNet(3,1)
        ...
        net.load_state_dict(torch.load('./best_model/seg/RCUNet/flod_' + str(i + 1) + '.pth'))
